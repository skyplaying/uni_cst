<template>
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			<view class="imageItem" :class="index%4==0?'imageItem-row-first':''" v-bind:key="index" v-for="(path,index) in value">
				<view @tap="previewImage" :data-index="index" @touchstart="start" @touchmove.stop="move" @touchend="stop">
					<image mode="aspectFill" :src="path" :class="{'dragging':isDragging(index)}" draggable="true"></image>
				</view>
				<view v-if="isShowDel" class="imageDel" @tap.stop="deleteImage" :data-index="index">×</view>
		...
		<image mode="aspectFill" v-if="showMoveImage" class="moveImage" :style="{left:posMoveImageLeft, top:posMoveImageTop}"
...
<script>
	...
	export default {
		...
		props: ['value', 'enableDel', 'enableAdd', 'enableDrag', 'serverUrl', 'formData', 'limit', 'fileKeyName',
			'showUploadProgress', 'firstImageLabel'
		],
		data() {
			return {
				imageBasePos: {
